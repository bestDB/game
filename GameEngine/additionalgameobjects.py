...
class Tile(SerializableObject):
    ...
    def is_mouse_over(self):
        ...
        if position!= None and  MathHelper.point_inside_polygon(position, \
                        MathHelper.move_points_by_shift(self.active_area, self.offX, self.offY)) :
            return True
        return False
...
class TiledSpace(Tile) :
    def __init__(self, width, height, showBorders = True, showNums = False):
        ...
        self.active_area = [(0,0), (width,0), (0, height), (width, height)]
    ...
    def on_focus_action(self,surface):
        while self.focused :
            ...
            isMouseOver = self.is_mouse_over()
            if index != None :
                self.unfocus()
    ...
    def add_tile(self, index, tileObject):
        ...
        if tileObject.active_area == None :
            tileObject.active_area = [self.tilesCoords[index][Corners.TOP_LEFT],\
                                      self.tilesCoords[index][Corners.TOP_RIGHT],\
                                      self.tilesCoords[index][Corners.BOTTOM_LEFT],\
                                      self.tilesCoords[index][Corners.BOTTOM_RIGHT]]
    ...
    def get_tile_index_from_mouse_position(self):
        ...
        if position != None :
            for tileNum in self.tilesCoords :
                if self.tiles[tileNum] != None and self.tiles[tileNum].is_mouse_over():
                    return tileNum
        return None
